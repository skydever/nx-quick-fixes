...
function readCliConfig() {
    var config = JSON.parse(fs.readFileSync(appRoot.path + "/.angular-cli.json", 'utf-8'));
...
function getAffectedApps(touchedFiles) {
    ...
    return affected_apps_1.affectedApps(config.project.npmScope, projects, function (f) { return fs.readFileSync(appRoot.path + "/" + f, 'utf-8'); }, touchedFiles);
}
exports.getAffectedApps = getAffectedApps;
function getTouchedProjects(touchedFiles) {
    return affected_apps_1.touchedProjects(getProjectNodes(readCliConfig()), touchedFiles).filter(function (p) { return !!p; });
}
exports.getTouchedProjects = getTouchedProjects;
function getProjectRoots(projectNames) {
    var projects = getProjectNodes(readCliConfig());
