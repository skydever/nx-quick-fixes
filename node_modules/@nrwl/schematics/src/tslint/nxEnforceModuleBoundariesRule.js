...
var Rule = /** @class */ (function (_super) {
    ...
    function Rule(options, projectPath, npmScope, libNames, appNames, roots) {
        ...
        if (!projectPath) {
            ...
            _this.roots = cliConfig.apps.map(function (a) { return path.dirname(a.root); });
        }
        return _this;
    }
    Rule.prototype.apply = function (sourceFile) {
        return this.applyWithWalker(new EnforceModuleBoundariesWalker(sourceFile, this.getOptions(), this.projectPath, this.npmScope, this.libNames, this.appNames, this.roots));
    };
...
var EnforceModuleBoundariesWalker = /** @class */ (function (_super) {
    ...
    function EnforceModuleBoundariesWalker(sourceFile, options, projectPath, npmScope, libNames, appNames, roots) {
        ...
        _this.appNames = appNames;
        _this.roots = roots;
        _this.roots = roots.slice().sort(function (a, b) { return a.length - b.length; });
        return _this;
    }
    ...
    EnforceModuleBoundariesWalker.prototype.isRelativeImportIntoAnotherProject = function (imp) {
        ...
        var targetFile = this.getTargetFile(imp, sourceFile);
        if (!this.libraryRoot())
            return false;
        return !(targetFile.startsWith(this.libraryRoot() + "/") || targetFile === this.libraryRoot());
    };
    EnforceModuleBoundariesWalker.prototype.getTargetFile = function (imp, sourceFile) {
        var targetFile = path.resolve(path.dirname(sourceFile), imp).substring(1); // remove leading slash
    ...
    EnforceModuleBoundariesWalker.prototype.libraryRoot = function () {
        var sourceFile = this.getSourceFile().fileName.substring(this.projectPath.length + 1);
        return this.roots.filter(function (r) { return sourceFile.startsWith(r + "/"); })[0];
    };
